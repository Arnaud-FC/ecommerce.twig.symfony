...
class ProductController extends AbstractController
{
    ...
    public function edit(Request $request, Product $product, EntityManagerInterface $entityManager, SluggerInterfacce $slugger): Response
    {
    ...
    public function addStock($id, EntityManagerInterface $entityManager, Request $request, ProductRepository $productRepository ):Response {
        ...
        if($form->isSubmitted() && $form->isValid()){
            ...
            if($addStock->getQte()>0){
                ...
                $this->addFlash('success', 'quantité actualisée');
                return $this->redirectToRoute('app_product_index');
            }
            else {
                $this->addFlash('danger', 'le stock ne doit pas etre négatif');
